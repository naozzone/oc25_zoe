...
robot.move(0, 0)
# Constantes positions pince (servo 1)
PINCE_OUVERTE = 160
PINCE_FERMEE = 20

# Pince ouverte (pour la refermer après pour attraper objet)
robot.goToPosition(1, PINCE_OUVERTE) 
...
def suivre_ligne(): # suivre la ligne 
...
def danser():
    ...
    robot.goToPosition(1, PINCE_OUVERTE) # ouvre pince 
    robot.goToPosition(1, PINCE_FERMEE) # ferme pince
...
while True:
    ...
    # Programme final autonome (prog == 8)
    if prog == 8:
       while True:
        if button_b.is_pressed():
            # 1. Préparation
            ...
            # 2. Suivre ligne jusqu'à détection d'objet
            while distance() > 20:
                suivre_ligne()
    
            robot.move (0,0)
            t1 = time.ticks_ms()
            dt = t1 - t0  # temps pour atteindre l'objet
            ...
            # 4. Retour en arrière (suivre ligne pendant dt)
            t2 = time.ticks_ms() + dt
            while time.ticks_ms() < t2:
                suivre_ligne()
            robot.move (0,0)
            ...
            # 5. Déposer l'objet
            robot.goToPosition(1, 160)  # ouvrir pince
            sleep(500)
